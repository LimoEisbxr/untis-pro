...
function mountCustomScrollbar() {
  ...
  const update = () => {
    ...
    const constrainedThumbHeight = Math.min(thumbHeight, availableHeight - 4)
    const maxTop = availableHeight - constrainedThumbHeight
    
    const top = scrollHeight > clientHeight ? 
      Math.max(0, Math.min(maxTop, Math.floor((scrollTop / (scrollHeight - clientHeight)) * maxTop))) : 0
  ...
  const onPointerMove = (e: PointerEvent) => {
    ...
    container.scrollTop = Math.max(0, Math.min(scrollable, startScrollTop + scrollDelta))
  }
  ...
  track.addEventListener('pointerdown', (e) => {
    ...
    const targetTop = Math.max(0, Math.min(maxTop, y - constrainedThumbHeight / 2))
    const scrollable = scrollHeight - clientHeight
    container.scrollTop = Math.max(0, Math.min(scrollable, (targetTop / maxTop) * scrollable))
  })
  update()
}
